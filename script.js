console.log("✅ KW BOB v3.1 script loaded");
...
if(page === "main.html"){
  ...
  const MENU_DB = [
    ...
    {restaurant:"마라탕",name:"마라탕(보통맵기)",price:9000,calorie:700,category:"중식"},
  ...
  function recommendMenu(){
    ...
    // 결과가 5개 미만일 경우, 다른 종류에서 채워넣기
    if(filtered.length < 5){
      const others = MENU_DB.filter(m => !filtered.includes(m));
      while(filtered.length < 5 && others.length){
        filtered.push(others.splice(Math.floor(Math.random()*others.length),1)[0]);
      }
    }
    ...
    randomMenus.forEach(m => {
      ...
      li.innerHTML = `${m.name} (${m.category}) - ${m.calorie} kcal ${(m.price?`/ ${m.price}원 / ${m.restaurant}`:'(자취요리)')}
      <br><button onclick="selectMenu('${m.name}',${m.calorie})">선택</button>`;
      list.appendChild(li);
    });
  ...
  function showRecords(){
    ...
    new Chart($("calorieChart"), {
      type: "bar",
      data: {
        labels: todayRecords.map(r => r.menu),
        datasets: [{label: "칼로리(kcal)", data: todayRecords.map(r => r.calorie), backgroundColor: "#007bff"}]
      },
